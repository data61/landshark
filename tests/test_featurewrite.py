...
def build_imagestack(parent_mock):
    width = 10
    height = 5
    image_stack = parent_mock.Mock()
    ...
    image_stack.categorical_blocks = parent_mock.Mock(return_value=block_it_ord)
    image_stack.ordinal_blocks = parent_mock.Mock(return_value=block_it_cat)
    return image_stack
...
def test_write_datafile(mocker, standardise):
    ...
    m_std_write = mocker.patch('landshark.importers.featurewrite._standardise_write')
    m_update_stats = mocker.patch('landshark.importers.featurewrite._update_stats')
    ...
    image_stack = build_imagestack(mocker)
    filename = 'filename'
