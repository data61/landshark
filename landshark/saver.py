"""Model saving and "wratcheting" functionality."""
...
class BestScoreSaver:
    ...
    def _to_64bit(self, scores: Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
    ...
    def _should_overwrite(self, s: str, score: np.ndarray, score_path: str) -> bool:
    ...
    def _write_score(
        self, scores: Dict[str, np.ndarray], score_path: str, global_step: int
    ) -> None:
        score_file = os.path.join(score_path, "model_best.json")
        with open(score_file, "w") as f:
            json.dump(scores, f)
        checkpoint_files = glob(
            os.path.join(self.directory, "model.ckpt-{}*".format(global_step))
        )
        checkpoint_files += [
        ...
        deleting_files += [
            os.path.join(score_path, "checkpoint"),
    ...
    def save(self, scores: dict) -> None:
        ...
        for s in scores.keys():
            ...
            if self._should_overwrite(s, scores[s], score_path):
                log.info("Found model with new best {} score: overwriting".format(s))
